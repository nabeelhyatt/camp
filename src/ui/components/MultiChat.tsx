...
export function ToolsMessageView({
    ...
    function onReplyClick() {
        if (message.replyChatId) {
            // Existing reply - open in sidebar drawer
            navigate(`/chat/${message.chatId}?replyId=${message.replyChatId}`);
        } else if (campConfig.useConvexData) {
            // In Convex mode, create a private reply and open in sidebar
            // This copies messages up to the fork point and opens in RepliesDrawer
    ...
    const handleToggleVisionMode = useCallback(async () => {
        ...
        if (!visionModeEnabled && !hasPermissions) {
            toast("Screen Recording Permission Required", {
                description:
                    "Camp needs screen recording permission to enable vision mode.",
                action: {
                    label: "Open Settings",
                    onClick: () => {
                    ...
                    <div className="flex items-center gap-2 ml-auto text-sm font-[350]">
                        <Tooltip>
                            ...
                            <TooltipContent>
                                {appMetadata["vision_mode_enabled"] ===
                                "true" ? (
                                    <>Camp can see your screen</>
                                ) : (
                                    <>
                                        Enable vision mode to show Camp your
                                        screen
                                    </>
                                )}
                            </TooltipContent>
